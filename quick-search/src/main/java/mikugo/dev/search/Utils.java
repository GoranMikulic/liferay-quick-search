...
public class Utils {
	...
	public static List<ResultModel> convertResult(List<Document> result,
		...
		for (Document document : result) {
			...
			String returnToFullPageURL = (String) request
					.getAttribute("search.jsp-returnToFullPageURL");
			...
			if (assetRendererFactory != null) {
				...
				downloadURL = assetRenderer.getURLDownload(Utils
						.getThemeDisplay(request));

				viewFullContentURL = getViewFullContentURL(request,
				...
				viewFullContentURL = getViewFullContentURL(request,
						Utils.getThemeDisplay(request), portletId, document);
			...
			if (indexer != null) {
				...
				Summary summary = indexer.getSummary(document, Utils
						.getThemeDisplay(request).getLocale(), snippet,
						viewFullContentURL);

				entryTitle = summary.getTitle();
				entrySummary = summary.getContent();
			} else if (assetRenderer != null) {
				entryTitle = assetRenderer.getTitle(Utils.getThemeDisplay(
						request).getLocale());
				entrySummary = assetRenderer.getSearchSummary(Utils
